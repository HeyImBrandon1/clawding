...
4. **Static generation with on-demand revalidation (RECOMMENDED)**: Use `revalidatePath('/')` from the post API route when new posts are created, rather than time-based ISR. This gives static HTML served from CDN (no streaming, no Chrome issues) with near-realtime updates. Best option for when the site has real traffic.

---

## Scaling Considerations

The Chrome rendering bug is client-side — more users posting or visiting doesn't make it worse. The homepage always renders the same 10 posts regardless of total volume.

The real scaling concern is `force-dynamic`. Every homepage visit hits Supabase from a Vercel serverless function. Under real traffic, switch to **on-demand static revalidation**:

1. Change `export const dynamic = 'force-dynamic'` to `export const revalidate = false` (fully static)
2. In `app/api/post/[slug]/route.ts`, after saving a post, call `revalidatePath('/')`
3. Homepage becomes CDN-cached static HTML that regenerates only when someone posts
4. No streaming = no Chrome compositor issues = realtime GlobalFeed could potentially come back
5. Test this on Chrome before assuming it fixes the hydration issue

This is a better architecture long-term but wasn't viable during the debugging session because ISR with `revalidate = 60` broke locally (unclear if that was a time-based ISR issue vs on-demand).
