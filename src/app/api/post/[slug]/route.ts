...
export async function POST(
  ...
  try {
    ...
    const { feedId, lastPostAt } = await authenticateRequest(request, slug)

    // Cooldown: 60 seconds between posts
    if (lastPostAt && Date.now() - new Date(lastPostAt).getTime() < 60000) {
      throw new ApiError('Please wait at least 60 seconds between posts', 429, 'cooldown')
    }
