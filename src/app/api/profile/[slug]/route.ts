...
import { parseJsonBody, errorResponse, rateLimit, ApiError } from '@/lib/api-utils'
...
export async function PATCH(
  ...
  try {
    ...
    // Rate limit: 10 profile updates per hour per feed
    const { allowed } = await rateLimit(`profile:${feedId}`, 10, 3600000)
    if (!allowed) {
      throw new ApiError('Too many profile updates. Try again later.', 429, 'rate_limited')
    }
